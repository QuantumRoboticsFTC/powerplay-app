...
//import eu.qrobotics.powerplay.teamcode.opmode.auto.trajectories.TrajectoriesLeftCtdTupeu;
...
//
//@Config
//@Autonomous(name = "#0 AutoLeftCtdTupeu")
//public class AutoLeftCtdTupeu extends LinearOpMode {
//    public static double EXTENDO_THRESHOLD = 0.5;
//    public static double EXTENDO_SENSOR_THRESHOLD = 4;
...
//    private double STOP_CYCLE_FOR_PARK_TIME = 1.95;
...
//        if (!sleepFailsafe(robot, 1.2)) return;
//
...
//        if (!sleepFailsafe(robot, 0.22)) return;
//
//        robot.extendo.manualPower = Extendo.autonomousGoBackAfterStack;
//        robot.extendo.extendoMode = Extendo.ExtendoMode.MANUAL;
//        if (!sleepFailsafe(robot, 0.23)) return;
//
//        robot.intake.armRotate = Intake.ArmRotate.TRANSFER;
//        robot.outtake.armPosition = Outtake.ArmPosition.TRANSFER;
...
//            robot.extendo.extendoMode = Extendo.ExtendoMode.AUTOMATIC;
//        }
//        if (!sleepFailsafe(robot, 0.175)) return;
//
...
//        }
//        robot.extendo.manualPower = 0;
//        if (!sleepFailsafe(robot, 0.125)) return;
//
//        robot.intake.clawMode = Intake.ClawMode.CLOSED;
//        if (!sleepFailsafe(robot, 0.2)) return;
//
...
//        robot.drive.setPoseEstimate(TrajectoriesLeftCtdTupeu.START_POSE);
//        robot.extendo.extendoMode = Extendo.ExtendoMode.RETRACTED;
//        robot.elevator.isScoring = false;
//        robot.elevator.scoringPosition = Elevator.TargetHeight.HIGH;
...
//        robot.outtake.bulanVar = 2.5;
//        robot.extendo.senzorDifValue = 3;
//        transferDone = true;
//        robot.start();
////        PhotonCore.enable();
//
...
//        autoTimer.reset();
//
//        List<Trajectory> trajectories = TrajectoriesLeftCtdTupeu.getTrajectories(readFromCamera);
...
//            robot.sleep(0.01);
//        }
//        if (!sleepFailsafe(robot, 2.5)) return;
//        intakeIteration = 1;
//        preload(robot);
...
//        if (!sleepFailsafe(robot, 0.25)) return;
//
//        for (intakeIteration = 2; intakeIteration <= 6; intakeIteration++) {
//            if (autoTimer.seconds() + STOP_CYCLE_FOR_PARK_TIME > 30) {
//                break;
//            }
//            cycle(robot);
